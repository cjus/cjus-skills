...
- `make-pdf` was the only one with teeth: a source-repo-only skill, cited in the verification step, and unreachable from any other repo after the port. Replaced the pointer with the venv commands it stood in for. Playwright stayed unvendored because it is already the *fallback* there, behind Claude-in-Chrome, which is not machine-scoped — vendoring would have backed up a path that survives the port intact. The scripted version is recorded under `## Deferred` in the plan.
