...
installed on one machine, and `qve` was a project skill inside the `cjus-dev` repo. `qe`
...
| `plugins/explain/skills/qve/SKILL.md` | Ported from `cjus-dev`; references rewritten through `${CLAUDE_PLUGIN_ROOT}`, dependencies reworded |
...
**A dependency on a skill that exists in exactly one repo.** `make-pdf` is `cjus-dev`-only and
...
`cjus-dev`. Retiring them is Phases 8 and 9, post-merge.
