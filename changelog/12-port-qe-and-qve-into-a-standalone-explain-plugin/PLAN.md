...
**Where they live today.** The two are not in the same place, and neither of them is a pair in the source repo:

- `qe` — `~/.claude/skills/qe/SKILL.md`, a **user-level** skill. It is not in the source repo or any other repo; it is installed on this machine only.
- `qve` — `~/dev/the source repo/.claude/skills/qve/`, a **project** skill in the source repo, with `references/page-template.html`.

That split is the reason for the port: `qe` works everywhere but only on machines it has been copied to, and `qve` works only inside the source repo.

**Goal: the plugin becomes the only home.** Once the plugin is deployed and verified, both originals are deprecated — the global `~/.claude/skills/qe/` and the project-level source repo `qve`. Two live copies of a skill drift, and a user-level `qe` shadowing a plugin `qe` is a confusing failure to diagnose.
...
- [ ] Phase 9 (post-merge): Deprecate the source repo's `.claude/skills/qve/` — remove it, and enable this plugin in the source repo so `/qve` keeps working there.
...
reference suffices. `make-pdf` is the only one with teeth, being source-repo-only and cited
