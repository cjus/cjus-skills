...
remaining test suites, and the retirement of the source repo copy.
...
- [ ] Phase 9: retire the source repo copy and relocate the A-060 warning. **Split at the repo
      boundary on 2026-09-18** — only the `detect.sh` half lands here; the source repo half is
...
- ~~**Phase 9 touches a second repo (the source repo).**~~ **Resolved 2026-09-18: split it.** The source repo half becomes its own ticket there, blocked on this PR. See
...
- [ ] **The defaults need a single declared home first.** They are currently nowhere in the
      ...
      defaults live in the source repo copy's `skills/ask/SKILL.md` prose. Declare them once —
...
Settled 2026-09-18. The source repo half becomes its own ticket in that repo, blocked on this PR.
...
in **the source repo**, a different repository. No branch here can carry those commits, so the window
...
- **Ordering.** Phase 9 verifies by running a real council from the *installed* plugin inside
  the source repo. Installation needs this merged, or a local-path install, so the verification cannot
...
**Moves to a source repo ticket**, blocked on this PR merging:
...
land as its own change there."* Settling it is the answer, not scope drift. **The source repo
ticket is a ticket in the source repo**, filed
...
`COUNCIL_OPENROUTER_API_KEY` is absent from the source repo's `.env`, from the environment and from
...
**It now travels with the source repo ticket**, not with this branch. Every step it corrects —
...
This is the same defect class the phase 9 notes already record for the source repo's
...
- [ ] **Phase 7** — `plugins/council/README.md`, the repo-root README section and Layout block.
- [ ] **Phase 8 (rest)** — `detect.sh` probes and `openrouter.mjs` exit-code tests.
- [ ] **Phase 9** — retire the source repo copy and relocate the A-060 warning.
