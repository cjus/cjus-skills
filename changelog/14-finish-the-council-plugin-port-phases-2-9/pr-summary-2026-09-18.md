...
| File | What it is |
|---|---|
| `plugins/council/skills/ask/SKILL.md` | Ported from the source repo's `council/SKILL.md`. The council itself: three reconciliation modes, the honesty contract, the trust boundary. |
...
```js
// before — plugins/council/scripts/openrouter.mjs (the source repo original)
function repoRoot() {
...
- **Phase 9 split at the repo boundary** before it ran. Its source repo half cannot live in a
  branch of this repo, so it became its own ticket there:
  **a ticket in the source repo**, filed 2026-09-18 and
  blocked on this PR.
...
- **The source repo copy is untouched** and still authoritative there. Retiring it is the split-out
