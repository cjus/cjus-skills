...
The retirement of the source repo copy is **not** part of this branch. Phase 9 splits at the
repo boundary — only its `detect.sh` half lands here, and the deletions in the source repo become
...
- **Phase 9 splits at the repo boundary.** Only the `detect.sh` half lands here; the source repo
  half becomes its own ticket there, blocked on this PR.

### 2026-09-18 — phase 1: the three skills, `reference/`, the marketplace entry

- **`skills/ask/SKILL.md`** — ported from the source repo's `council/SKILL.md`. Renamed to
...
- **`skills/status/SKILL.md`** — new, and the only skill without a source repo original. A
...
- **Repo-private material removed** from every ported file. The `ASSERTIONS.md → A-060`
  attribution, the `sb's PRODUCTION key` wording, the hash-verified same-secret note and the source repo settings claim are all gone; the generic halves of those warnings are kept.
