...
- **Open, needs the operator:** Phase 1's live half needs a restart, which ends the session that would run it. The verification therefore has to happen in a fresh session started from a repo other than `cjus-skills` — `cjus-dev` is the natural choice, since it also sets up Phase 4.
