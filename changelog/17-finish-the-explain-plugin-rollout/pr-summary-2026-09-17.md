...
work itself landed on the operator's machine and, for phase 4, in the source repo as
commit `f7bc581`.
...
- **Phase 1 — install and verify.** Done. `claude plugin marketplace update cjus-skills`
  ...
  Verified from the source repo: both skills resolve, and `qve` and `explain:qve` appear as
  distinct entries. **Recorded precisely:** resolution was observed in the source repo, while
...
- **Phase 4 — remove the source repo's `qve`.** Done, landed as `f7bc581` on that repo's `main`
  ...
  scope and the source repo declares no project-level `enabledPlugins`.
...
the source repo's project-level `qve` correctly does not load at all. **No such rule exists.**
...
- **Breaking change, outside this repo:** the bare `/qe` and `/qve` no longer exist. Both
  are now `/explain:qe` and `/explain:qve`. the source repo's documentation is updated in
...
the `qve` removal in the source repo, and the stale `/qve` and `/qe` references at that repo's
`CLAUDE.md:416`.
...
- the source repo documents `CJUS_ALLOW_MAIN=1` as the approval token for its `main` gate and
