...
### 2026-09-20 — the two review gates

`/pr:pre-test` returned APPROVE with six suggestions; all six were verified against source
and applied in `b81f036`. The `/pr:close` review gate then returned APPROVE, independently
re-verified those six, and found three more accuracy defects in the new README, all fixed:

- The `on-session-start.sh` row claimed the hook loads "the plan folder and newest summary".
  It loads **all** summaries, and **instead of** the plan — the script's own comment says so,
  and `PLAN.md`/`CHANGELOG.md` are the `else` branch taken only when no summary exists.
- The `/pr:start` prose transposed two step pairs against `start/SKILL.md`: carry-over is
  step 3 and install is step 4, and reading continuity is step 6 before the step-7 enrich.
- The stated prefix convention ("drops it when naming a skill in prose") was not the one the
  page follows. Reworded to describe actual practice.
