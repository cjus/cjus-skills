...
def main(argv):
    ...
    for name, val in declared.items():
        ...
        #
        # The object form has to be unpacked here rather than tolerated. A dict
        # iterates over its KEYS, so the old line read "path" and "display" as
        # two relative paths, resolved both against the book folder, and
        # reported each as a declared source that does not exist. Two
        # fabricated failures per display-named source, and the real path never
        # opened.
        if isinstance(val, dict):
            raw = val.get("path")
            if raw is None:
                problem(f"book.json declares \"{name}\" as an object with no "
                        f"\"path\" key", repr(val)[:80])
                continue
        else:
            raw = val
        if isinstance(raw, str):
            ...
            problem(f"book.json declares \"{name}\" with a path that is "
                    f"neither a string nor a list of strings", repr(raw)[:80])
            continue
        sources[name] = Source(name, [book / r for r in rel])
