...
- **`tag` is the paragraph tag** where the unit has one, taken from the head of the unit's text, as written: `[5-1]`, an appendix's `[A2-4]`, or a revision's lettered `[5-1a]`. Omit it for a table or list that carries none.
