...
def load_chapters(src: Path, skip: set[str]) -> list[dict]:
...
def figurize(body_html: str, chapter_num: int, counter: list[int],
             figures: list[dict], src: Path) -> str:
...
def build_chapters(chapters, cfg, figures: list[dict], src: Path,
    ...
    for ch in chapters:
        ...
        out.append(
            ...
            f'<span class="chapter-number">Chapter {ch["num"]}</span>'
...
def build_glossary(entries) -> str:
    ...
    for term, ch, body in entries:
        ...
        blocks.append(
            ...
            f'<span class="gloss-term">{html_mod.escape(term)}</span>{ref} '
            f'{html_mod.escape(body)}</div>'
        )
    return (
...
def assemble(title, cfg, chapters, src, state, want_index, css: str,
    ...
    """Returns [html, figures]. Figures are discovered while building the
    ...
    """
    figures: list[dict] = []
    chapter_html = build_chapters(chapters, cfg, figures, src, col_plan)
    parts = [
        "<!DOCTYPE html>",
    ...
    for term, ch, body in entries:
        ...
        if ch:
            ref = (f' <a href="{epub_chapter_href(ch)}" title="'
                   ...
                   f'ch. {ch}</a>')
        blocks.append(
            f'<p class="entry"><strong>{html_mod.escape(term)}</strong>{ref} '
            f'{html_mod.escape(body)}</p>'
        )
    return (
        "<h1>Glossary</h1>"
...
def build_epub(title, cfg, chapters, src, out_path, terms, want_index,
    ...
    for ch in chapters:
        ...
        section = group_for(ch["num"], cfg.get("sections") or [])
        eyebrow = (f'<p class="lead">Chapter {ch["num"]}'
...
def main(argv: list[str]) -> int:
    ...
    chapters = load_chapters(src, skip)
    if not chapters:
    ...
    for attempt in range(1, MAX_PASSES + 1):
        page_html, figures = assemble(args.title, cfg, chapters, src,
