...
if [[ -d "$WT" ]]; then
  ...
  # Both worktree cases are skipped, not one. Counting them keeps the totals line
  # honest: a run that silently drops cases while reporting "failed 0" is exactly
  # the ambiguity this suite's documented case count exists to remove.
  SKIP=$((SKIP+2))
  printf '  skip %-56s (worktree add failed)\n' "linked worktree resolves main config"
  printf '  skip %-56s (worktree add failed)\n' "linked worktree, feature commit passes"
fi
