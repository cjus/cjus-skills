...
**Plugin skills are namespaced by their plugin**, so every command is `/pr:<name>` — `/pr:close`, `/pr:next`. A bare `/close` does not resolve. This page writes the prefix out in every command you would type, and drops it when naming a skill in prose.
...
Creates the branch — in a linked worktree when `worktrees.enabled` is true — installs dependencies, carries over what a fresh checkout does not bring (untracked local config and the like), initializes the plan folder, enriches `PLAN.md` from the issue body, reads the recent continuity entries, and moves the issue to `status:in-progress`.
...
|---|---|---|
| `on-session-start.sh` | `SessionStart` | Loads the branch's plan folder and newest summary into context, so a session after a compaction does not resume blind. Silent on the default branch. |
