...
`commit` and `pr` are attribution **text**, so an empty string hides each one. `sessionUrl` is a
boolean that defaults to **true**, and leaving it out is what still emits the `Claude-Session:`
trailer and PR-body link this section bans, so all three keys are needed rather than the first
two. This repo sets them in its own `.claude/settings.json`.
