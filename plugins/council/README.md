...
The `reference/` documents are what keep the three skills repo-agnostic — the roster format, the provider invocations and the trust boundary are each documented once between them, owned by the plugin rather than the repo you are working in.
